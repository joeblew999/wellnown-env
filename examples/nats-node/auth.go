...
func LoadAuthConfig() (*AuthConfig, error) {
	cfg := &AuthConfig{
		Mode: env.GetEnv("NATS_AUTH", "none"),
	}
...
func ConfigureAuth(opts *server.Options, cfg *AuthConfig) error {
	switch cfg.Mode {
	case "none":
		fmt.Println("Auth: NONE (dev mode - no authentication)")
		return nil

	case "token":
		opts.Authorization = cfg.Token
		fmt.Println("Auth: TOKEN (test/CI mode)")
...
func configureNKeyAuth(opts *server.Options, cfg *AuthConfig) error {
	fmt.Println("Auth: NKEY (staging mode)")
...
func configureJWTAuth(opts *server.Options, cfg *AuthConfig) error {
	fmt.Println("Auth: JWT (production mode)")
	fmt.Printf("  Creds Dir: %s\n", cfg.CredsDir)
	...
	for _, entry := range entries {
		...
		if jwtBytes, err := os.ReadFile(accountJWTFile); err == nil {
			...
			if err == nil {
				...
				fmt.Printf("  Loaded account: %s (%s)\n", accountName, accountClaims.Subject[:12]+"...")
			}
		}
	}
